...
def cw_vpc_flow_log_enable(event, context):
    ...
    log_name = 'ironnet-vpc-flow-log-'+str(sts.get_caller_identity().get('Account'))+'/'+region+'/'+str(vpc_id)
...
def s3_vpc_flow_log_enable(event, context):
    if "boot_vpc_logs" != context:
        event = event['Records'][0]['Sns']['Message'].split(",")
    region = event[0].strip('"')
    vpc_id = event[1].strip('"\n')
